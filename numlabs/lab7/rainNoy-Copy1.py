...
def first_time_step(u, v, h, g, H, dt, dx, ho, gu, cor, gh, n_grid):
    ...
    v.now[1:n_grid - 1] = 0
    midpoint = n_grid // 2
    # u and v are at the midpoint from the first timestep (cor * v.prev[midpoint-1])-
    factor_u = (gu * ho /2)
    factor_v = -(cor * u.prev[midpoint-1])-(gu*ho)
...
def leap_frog(u, v, h, gu, cor, gh, n_grid):
...
#     n_time, n_grid = map(int, args)
    ...
    first_time_step(u, v, h, g, H, dt, dx, ho, gu, gh, cor, n_grid)
    ...
    for t in np.arange(2, n_time):
        ...
        leap_frog(u, v, h, gu, gh, cor, n_grid)

...
def leap_frog(u, v, h, gu, uf, vf, gh, n_grid):
    ...
    for pt in np.arange(1, n_grid - 1):
        u.next[pt] = u.prev[pt] + (uf * v) - gu * (h.now[pt + 1] - h.now[pt - 1]) # done -staggered 
        v.next(pt) = v.prev[pt] - (vf * u) # done - staggered
...
#     n_time, n_grid = map(int, args)
    ...
    # results arrays
    initial_conditions(u, v, h, ho) # still have to do this part
    ...
    first_time_step(u, v, h, g, H, dt, dx, ho, gu, gh, uf, vf, n_grid) # still have to discritize   d i s c   e q u  
    ...
    for t in np.arange(2, n_time):
        ...
        leap_frog(u, v, h, gu, gh, uf, vf, n_grid)
